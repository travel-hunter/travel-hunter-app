...
- 기준일: 2026-06-05 (인코딩 손상 복구 및 현재 상태 재조정 기준)
- 기준 검증 기준: 현재 `develop` 작업트리, 최근 검증일 2026-06-04 (`CHECKLIST.md` 기준)
- 브랜치: `develop`
- 원칙: DB-backed-only MVP를 유지하고 runtime mock mode는 다시 추가하지 않는다.
- 현재 단계: 도메인 독립 로컬 UX 완성이 현재 우선순위다. 인증/계정 복구, 정책-일정 연결, 장소 탐색 로컬 UX를 진행했고, Cloudflare/SMTP/OAuth provider 실 smoke와 배포/CICD는 실제 env 준비 시까지 보류한다. 활성 우선순위는 `docs/specs/spec-index.md`를 기준으로 한다.
...
작업 우선순위는 `docs/specs/spec-index.md`와 `docs/next-work-plan.md`를 기준으로 한다. 다음 구현은 로컬 수동 수집 명령, local recommendation smoke 스크립트, backend/frontend/e2e 검증 보강 순서로 진행한다.
...
- `docs/next-work-plan.md`: 다음 작업 우선순위(참고용, 활성 기준은 spec-index).
- `docs/specs/spec-index.md`: 현재 로컬 UX 활성 명세 인덱스(활성 source of truth).
- `docs/specs/local-ux-auth-account.md`, `docs/specs/local-ux-policy-trip-linking.md`, `docs/specs/local-ux-place-discovery.md`: 로컬 UX 도메인별 완성 명세.
- `docs/deployment-cicd/README.md`: 팀 배포/CICD 기준 문서.
- `docs/deployment-cicd/09-release-checklist.md`: 배포 전후 smoke와 rollback 체크리스트.

## 최신 검증 기록

기준일 2026-06-04 검증 (running ledger는 `CHECKLIST.md`).

- Frontend typecheck: passed.
- Frontend Vitest: `6 files / 149 passed`.
- Frontend e2e: passed.
