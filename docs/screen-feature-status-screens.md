...
홈 화면은 로그인 후 사용자가 정책 탐색과 일정 생성으로 진입하는 대시보드 역할로 개발되어 있다. 상단 검색 pill은 실제 홈 내 검색 입력이 아니라 정책 목록 화면으로 이동하는 버튼이며, 마이페이지 버튼은 사용자 아바타 형태로 제공된다. 인기 정책/이번 주 혜택 정책 카드는 실제 정책 API 데이터를 사용해 정책 상세로 연결되고, 인기 국내 여행지는 지역 추천 API 또는 정책 기반 fallback 데이터를 사용해 새 일정 생성 화면으로 연결된다. AI 추천 맞춤 일정 영역은 관심 지역이 있으면 관심 지역만 일정 생성 CTA로 보여주며, 2개 이상일 때는 순환형 카드 캐러셀로 제공된다. 공통 상단/하단 내비게이션도 홈 화면에서 동작한다.
