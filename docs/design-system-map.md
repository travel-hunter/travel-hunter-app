...
| --- | --- | --- |
| Button `Button/Button` | `frontend/src/components/ui.tsx`의 `Button`, `LinkButton` | Figma는 Large `48px`, Medium `40px`, Small `32px`, radius `12/10/8`, primary `#0066ff` 기준. 현재 앱은 기본 `46px`, radius `8px`, primary `#3366ff`라 2차 보정 결정 필요 |
...
- Button primary color와 radius를 Wanted 원본에 더 가깝게 맞출지, 현재 Travel Hunter 브랜드 톤을 유지할지 결정해야 한다.
