...
- 현재 기준은 profile DB persistence, 정책 담기 sheet, 일정 생성 `region/style/policySlug` payload, 친구 초대 수락 DB-backed 처리, backend-mode e2e CI 분리 실행, saved-policies DB persistence까지 포함한다.
...
| 우선순위 | 작업 | 성공 기준 |
|----------|------|-----------|
| 1 | 배포 readiness 정리 | staging build, env 분리, container validation 기준을 문서화하고 검증한다. |
| 2 | 정책 신청 deep link 처리 | 정책 상세의 공식 신청 연결을 실제 URL/deep link 기준으로 정리한다. |
