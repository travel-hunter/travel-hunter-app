...
| 우선순위 | 작업 | 성공 기준 |
|---:|---|---|
| 1 | Figma import 및 component variant 확인 | `.fig`를 Figma에 import하고 Button/Input/Badge/Card/Navigation/Sheet variant 수치를 `docs/design-system-map.md`와 대조한다. |
| 2 | Figma 비교 기반 2차 보정 | Figma 수치와 현재 앱이 충돌하는 부분만 token/component 중심으로 보정한다. |
| 3 | VPS 배포 입력값 확보 | VPS SSH, domain/DNS, repo clone 권한, `deploy/.env.staging` 값이 준비된다. |
