...
| 8 | 후속 기능 개발 의뢰 | 홈 추천 ranking 후속 개선, 전화번호 OTP 실제 발송 smoke 기준 확정 |
...
### DEV-02. 전화번호 OTP 실제 발송 smoke

목적: env-gated SOLAPI SMS provider로 구현된 전화번호 OTP 흐름을 실제 staging env에서 제한적으로 검증한다.

의뢰 전 확정할 것:

- `PHONE_VERIFICATION_PROVIDER=solapi` 적용 시점.
- 1일 발송 제한, 재전송 간격, 테스트 대상 번호 수.
- provider 실패/timeout 시 사용자 노출 메시지와 rollback 기준.

완료 증거:

- SOLAPI API key/secret과 발신번호 secret 보관 위치.
- 개발 PR.
