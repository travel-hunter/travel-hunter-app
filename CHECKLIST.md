...
- Latest validated scope: production release preflight for `promote-develop-to-main` plus existing development-server runtime smoke on `dev.travel-hunter.co.kr`.
- Last validation date: 2026-06-12.
- Current release posture: `origin/develop` at `09f3a4dfe6673080c4c049ae849e60b707281084` is deployed on the development server. Public/API/auth/trip/policy/invite/Kakao Maps, admin ops, SMTP send-trigger and inbox receipt, real Google/Kakao OAuth callback/session smoke, and Docker default-bridge/build DNS fix verification passed. Production release preflight found and fixed frontend audit/e2e release-gate drift; public production DNS/Cloudflare authority remains the external deployment blocker before production stack mutation.
