...
def collect_external_benefits_from_live_sources(
    ...
    *,
    ...
    source_results: list[SourceCollectionResult] = []
    all_rows = []
    for source_category, url in _source_urls().items():
        try:
...
def _source_urls() -> dict[str, str]:
    return {
        "regional_benefit": TRAVELMONTH_REGIONAL_BENEFIT_URL,
        "traffic_benefit": TRAVELMONTH_TRAFFIC_BENEFIT_URL,
        "local_half_trip": DGTOURCARD_URL,
    }
